...
- websocket_manager.py para gifts (via Zuros Cloud API)
...
def register_cloud_handlers():
    ...
    async def get_config(bot, payload: dict) -> dict:
        """Retorna a configuracao segura da Zuros Auth."""
        from modules.cloud.zuros_cloud import get_zuros_credentials, is_zuros_configured
        credentials = get_zuros_credentials()
        cloud_config = db.get_document("cloud_data") or {}
        cargos = db.get_document("cargos") or {}
        return {
            "config": {
                "auth_id": credentials.get("auth_id"),
                "integration_key_configured": bool(credentials.get("bot_credential")),
                "backend_url": credentials.get("base_url"),
                "log_channel_id": cloud_config.get("log_channel_id"),
                "verified_role_id": cargos.get("cargo_verificado"),
            },
            "is_configured": is_zuros_configured(),
            "transport": "rest_polling",
        }
    async def update_credentials(bot, payload: dict) -> dict:
        from modules.cloud.zuros_cloud import ZurosCloud, ZurosCloudError, save_zuros_credentials
        auth_id=str(payload.get('auth_id') or '').strip()
        credential=str(payload.get('bot_credential') or payload.get('integration_key') or '').strip()
        if not auth_id or not credential:
            raise ValueError('auth_id e bot_credential são obrigatórios')
        candidate=ZurosCloud('https://zuros-auth.vercel.app',auth_id,credential)
        try:
            status=await candidate.status()
        except ZurosCloudError as error:
            raise ValueError(f'Zuros Auth recusou a key: {error}') from error
        finally:
            await candidate.close()
        save_zuros_credentials(auth_id,credential)
        return {'success':True,'configured':True,'verified_count':int(status.get('verified_count',0))}

    async def update_log_channel(bot, payload: dict) -> dict:
        """Update log channel for Zuros Cloud"""
        try:
    ...
    async def update_verified_role(bot, payload: dict) -> dict:
        """Update verified role for Zuros Cloud"""
        try:
    ...
    async def get_stats(bot, payload: dict) -> dict:
        """Retorna estatisticas diretamente da Zuros Auth REST."""
        from modules.cloud.zuros_cloud import ZurosCloudError, get_zuros_cloud, is_zuros_configured
        if not is_zuros_configured():
            return {"verified_members": 0, "is_configured": False, "transport": "rest_polling"}
        try:
            total = await get_zuros_cloud().auth_count()
            return {"verified_members": total, "is_configured": True, "transport": "rest_polling"}
        except ZurosCloudError as error:
            return {"verified_members": 0, "is_configured": True, "transport": "rest_polling", "error": str(error)}
    ...
    # ============= GIFTS HANDLERS (via Zuros Cloud API) =============
    
    async def get_gifts(bot, payload: dict) -> dict:
        """Get all gifts from Zuros Cloud API"""
        try:
            cloud_config = db.obter("database/cloud/data.json") or {}
            ...
            if not ws_manager.is_connected():
                raise ValueError("WebSocket not connected to Zuros Cloud")
            
            # Get gifts from Zuros Cloud API
    ...
    async def get_gift(bot, payload: dict) -> dict:
        """Get specific gift from Zuros Cloud API"""
        try:
    ...
    async def create_gift(bot, payload: dict) -> dict:
        """Create new gift via Zuros Cloud API"""
        try:
            ...
            # Create gift via Zuros Cloud API
    ...
    async def update_gift(bot, payload: dict) -> dict:
        """Update gift via Zuros Cloud API"""
        try:
            gift_id = payload.get('gift_id')
            gift_data = payload.get('gift_data')
    ...
    async def delete_gift(bot, payload: dict) -> dict:
        """Delete gift via Zuros Cloud API"""
        try:
            gift_id = payload.get('gift_id')
        ...
        try:
            ...
            if style == "embed":
                ...
                button = disnake.ui.Button(
                    label=button_data.get("label") or "Verificar",
    ...
    async def connect_websocket(bot, payload: dict) -> dict:
        """Connect to Zuros Cloud WebSocket"""
        try:
            from modules.cloud.update_api import get_websocket_manager
            ws_manager = get_websocket_manager()
    ...
    async def disconnect_websocket(bot, payload: dict) -> dict:
        """Disconnect from Zuros Cloud WebSocket"""
        try:
            from modules.cloud.update_api import get_websocket_manager
            ws_manager = get_websocket_manager()
