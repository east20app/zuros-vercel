...
async def log_restart(bot: disnake.Client):
    try:
        ...
        description_text_embed = (
            f"{emoji.reload} O **Zuros Bot** foi reiniciado.\n"
        ...
        description_text_container = (
            f"{emoji.reload} O **Zuros Bot** foi reiniciado @everyone\n"
            f"{emoji.robot} Versão atual: `{info['version']}`\n"
            f"{emoji.calendar} **Data:** <t:{int(time.time())}:f> (<t:{int(time.time())}:R>)"
        )
        ...
        buttons = disnake.ui.ActionRow(
            disnake.ui.Button(
                ...
                url=f"https://api.zurosapplications.com.br/dashboard"
            ),
            disnake.ui.Button(
                label="Suporte",
                style=disnake.ButtonStyle.link,
                url=f"https://api.zurosapplications.com.br/social/discord"
            )
        )
