"""
Zuros Wallet Payment Integration
Integração com Zuros Pay API para pagamentos PIX e gerenciamento de carteira virtual
"""
...
# URL base da Zuros Pay API (pode ser configurada via env ou settings)
def _get_zuros_pay_url() -> str:
    """Retorna a URL base da Zuros Pay API"""
    # Tentar pegar do .env primeiro
    url = "https://api.zurospay.com.br"
    return url.rstrip("/")
...
def _sanitize_error_message(error_msg: str) -> str:
    ...
    if not msg or len(msg) < 3:
        return "Erro ao processar com Zuros Wallet. Verifique as configurações."
...
async def _request(
    ...
    """Faz requisição HTTP para a Zuros Pay API"""
    base_url = _get_zuros_pay_url()
    url = f"{base_url}/{path}"
    ...
    async with aiohttp.ClientSession(timeout=t) as session:
        async with session.request(method, url, json=payload, headers=headers) as resp:
            ...
            if data is None:
                raise RuntimeError("Resposta inválida do servidor Zuros Pay")
...
def _require(value: Optional[str], what: str) -> str:
    ...
    if not value:
        raise ValueError(f"Missing {what} in Zuros Wallet settings.")
    return value


def _get_api_key() -> str:
    """Retorna a API Key da Zuros Wallet das configurações"""
    config = _load_config()
    zuros_config = config.get("zuros_wallet") or {}
    api_key = zuros_config.get("api_key")
    return _require(api_key, "Zuros Wallet API Key")


# ==================== PAGAMENTOS ====================

async def create_zuros_payment(
    api_key: str,
    value: float,
    ...
    """
    Cria uma cobrança PIX via Zuros Wallet
    
    Args:
        api_key: API Key do usuário na Zuros Wallet
    ...
    if result.get("success") and result.get("data"):
        ...
        if qrcode_url:
            ...
            if isinstance(qrcode_url, str) and qrcode_url.startswith("data:image"):
                try:
                    ...
                    print(f"❌ Zuros Wallet erro ao decodificar QR base64: {e}")
        ...
        if pix_code:
            try:
                ...
                print(f"❌ Zuros Wallet erro ao gerar QR customizado: {e}")
...
async def check_zuros_payment(api_key: str, payment_id: str) -> Dict[str, Any]:
    """
    Verifica o status de um pagamento na Zuros Wallet
...
async def cancel_zuros_payment(api_key: str, payment_id: str) -> Dict[str, Any]:
    """
...
async def refund_zuros_payment(
    api_key: str,
    payment_id: str,
...
async def create_zuros_withdraw(
    api_key: str,
    amount: float,
...
async def get_zuros_withdraw(api_key: str, withdraw_id: str) -> Dict[str, Any]:
    """
...
async def list_zuros_withdraws(
    api_key: str,
    status: Optional[str] = None,
    limit: int = 50,
    offset: int = 0,
...
async def register_zuros_user(
    ...
    """
    Registra um novo usuário na Zuros Wallet
...
async def get_zuros_user(api_key: str) -> Dict[str, Any]:
    """
...
async def get_zuros_balance(api_key: str) -> Dict[str, Any]:
    """
...
async def create_zuros_payment_from_settings(
    value: float,
    description: Optional[str] = None,
    ...
    if cover_fee is None:
        config = _load_config()
        zuros_config = config.get("zuros_wallet") or {}
        cover_fee = zuros_config.get("cover_fee", False)
    
    return await create_zuros_payment(api_key, value, desc, cover_fee)


async def check_zuros_payment_from_settings(payment_id: str) -> Dict[str, Any]:
    """Verifica pagamento usando API Key das configurações"""
    api_key = _get_api_key()
    return await check_zuros_payment(api_key, payment_id)


async def list_zuros_payments_from_settings(
    status: Optional[str] = None,
    limit: int = 50,
    offset: int = 0
) -> Dict[str, Any]:
    ...
    api_key = _get_api_key()
    return await list_zuros_payments(api_key, status, limit, offset)


async def cancel_zuros_payment_from_settings(payment_id: str) -> Dict[str, Any]:
    """Cancela pagamento usando API Key das configurações"""
    api_key = _get_api_key()
    return await cancel_zuros_payment(api_key, payment_id)


async def refund_zuros_payment_from_settings(
    payment_id: str,
    reason: Optional[str] = None
) -> Dict[str, Any]:
    """Reembolsa pagamento usando API Key das configurações"""
    api_key = _get_api_key()
    return await refund_zuros_payment(api_key, payment_id, reason)


async def create_zuros_withdraw_from_settings(
    amount: float,
    pix_key: Optional[str] = None,
    pix_key_type: Optional[str] = None
) -> Dict[str, Any]:
    ...
    api_key = _get_api_key()
    return await create_zuros_withdraw(api_key, amount, pix_key, pix_key_type)


async def get_zuros_withdraw_from_settings(withdraw_id: str) -> Dict[str, Any]:
    """Busca saque usando API Key das configurações"""
    api_key = _get_api_key()
    return await get_zuros_withdraw(api_key, withdraw_id)


async def list_zuros_withdraws_from_settings(
    status: Optional[str] = None,
    limit: int = 50,
    offset: int = 0,
    zuros: bool = False
    ...
    api_key = _get_api_key()
    return await list_zuros_withdraws(api_key, status, limit, offset, zuros)


async def get_zuros_user_from_settings() -> Dict[str, Any]:
    """Busca dados do usuário usando API Key das configurações"""
    api_key = _get_api_key()
    return await get_zuros_user(api_key)


async def update_zuros_user_from_settings(
    name: Optional[str] = None,
    email: Optional[str] = None,
    phone: Optional[str] = None,
    pix_key: Optional[str] = None,
    ...
    api_key = _get_api_key()
    return await update_zuros_user(api_key, name, email, phone, pix_key, pix_key_type)


async def get_zuros_balance_from_settings() -> Dict[str, Any]:
    """Busca saldo usando API Key das configurações"""
    api_key = _get_api_key()
    return await get_zuros_balance(api_key)


__all__ = [
    # Pagamentos
    "create_zuros_payment",
    "check_zuros_payment",
    "list_zuros_payments",
    "cancel_zuros_payment",
    "refund_zuros_payment",
    # Saques
    "create_zuros_withdraw",
    "get_zuros_withdraw",
    "list_zuros_withdraws",
    # Usuário
    "register_zuros_user",
    "get_zuros_user",
    "update_zuros_user",
    "get_zuros_balance",
    # Wrappers com settings
    "create_zuros_payment_from_settings",
    "check_zuros_payment_from_settings",
    "list_zuros_payments_from_settings",
    "cancel_zuros_payment_from_settings",
    "refund_zuros_payment_from_settings",
    "create_zuros_withdraw_from_settings",
    "get_zuros_withdraw_from_settings",
    "list_zuros_withdraws_from_settings",
    "get_zuros_user_from_settings",
    "update_zuros_user_from_settings",
    "get_zuros_balance_from_settings",
]
