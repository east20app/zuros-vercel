...
class BoostCommands(commands.Cog):
    ...
    async def boost_enviar(
        ...
        if check_plan_restriction():
            return await inter.send("❌ Este comando não está disponível para o seu plano atual (Cloud).", ephemeral=True)
