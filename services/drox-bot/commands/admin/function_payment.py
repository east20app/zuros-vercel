...
def _configured_providers() -> List[str]:
    ...
    # Zuros Wallet
    if has((cfg.get("zuros_wallet") or {}).get("api_key")):
        configured.append("zuros_wallet")
    # PIX Manual
...
class PaymentCog(commands.Cog):
    ...
    async def pagamento(
        ...
        if not should_allow_payment_provider(key):
            await embed_message.error(
                inter, 
                f"{emoji.information} No plano atual, apenas **Zuros Wallet** está disponível como forma de pagamento. Para usar outros métodos, faça upgrade do seu plano."
            )
            return
        ...
        try:
            if key == "zuros_wallet":
                ...
                data = await create_zuros_payment_from_settings(valor, description=descricao or f"Pagamento para {user.display_name}")
            ...
            error_msg = re.sub(r'payments\.zurosapplications\.com\.br[^\s]*', '', error_msg, flags=re.IGNORECASE)
        ...
        try:
            for _ in range(180):
                ...
                try:
                    ...
                    elif key == "zuros_wallet":
                        chk = await check_zuros_payment_from_settings(payment_id)
                    else:
                        chk = {}
                except Exception:
                    chk = {}
