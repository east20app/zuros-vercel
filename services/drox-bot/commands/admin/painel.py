...
class PainelCommand(commands.Cog):
        ...
        return [
            disnake.ui.Container(
                ...
                disnake.ui.ActionRow(
                    ...
                    disnake.ui.Button(label="PromisseCloud", style=disnake.ButtonStyle.grey, emoji=emoji.cloud, custom_id="Painel_Cloud", disabled=not button_states["cloud"]),
                ),
                disnake.ui.ActionRow(
    ...
    def PainelEmbed(self, inter: disnake.MessageInteraction, primary_color_hex: str = None, button_states: dict = None):
        embed = disnake.Embed(
            title=f"Painel",
            description=f"Olá senhor(a) **{inter.user.name}**, {self._get_salutation()} \n-# Aqui você pode **configurar** e **personalizar** as funcionalidades do seu **Promisse Bot**.",
        )
        if primary_color_hex:
            primary_color = int(primary_color_hex.replace("#", ""), 16)
