...
class DroxGenConfig(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # However, the user's config.json has "https://api.zurosapplications.com.br" which might be wrong or old.
        ...
        return [
            disnake.ui.Container(
                disnake.ui.TextDisplay(
                    f"# {emoji.zuros}\n"
                    f"-# Painel > Configurações > **Drox Gen**\n"
                ),
    ...
    def panel_embed(self, inter: disnake.MessageInteraction) -> tuple:
        ...
        embed = disnake.Embed(
            title=f"{emoji.commands} Drox Gen",
            description=(
                f"-# Painel de controle completo para o Drox Gen."
                f"{emoji.on} **Status:** Ativo\n"
                f"{emoji.member} **Usuário Integrado:** `{integrated_user}`\n"
    ...
    async def on_zurosgen_dropdown(self, inter: disnake.MessageInteraction):
        if inter.component.custom_id == "DroxGen_Select":
        ...
        elif inter.component.custom_id == "DroxGen_SystemSelect":
            choice = inter.values[0]
        ...
        elif inter.component.custom_id == "DroxGen_ProjectSelect":
            val = inter.values[0]
                        ...
                        components = [
                            ...
                            disnake.ui.ActionRow(
                                disnake.ui.Button(
                                    ...
                                    style=disnake.ButtonStyle.grey,
                                    emoji=emoji.back,
                                    custom_id="DroxGen_BackToMain" # Needs handler to go back to main DroxGen panel
                                )
                            )
                        ]
    ...
    async def _configure_system_menu(self, inter: disnake.MessageInteraction):
        ...
        components = [
            disnake.ui.Container(
                ...
                disnake.ui.ActionRow(
                    disnake.ui.StringSelect(
                        custom_id="DroxGen_SystemSelect",
    ...
    async def _show_gen_category_selection(self, inter: disnake.MessageInteraction):
        ...
        components = [
            disnake.ui.ActionRow(
                disnake.ui.StringSelect(
                    custom_id="DroxGen_CategorySelect",
    ...
    async def on_zurosgen_button_click(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        
        if custom_id == "DroxGen_BackToMain":
             await self.display_panel(inter) 
        
        elif custom_id == "DroxGen_BackToSystem":
             await self._configure_system_menu(inter)

        elif custom_id.startswith("DroxGen_OpenServiceModal_"):
            category = custom_id.replace("DroxGen_OpenServiceModal_", "")
        ...
        elif custom_id == "DroxGen_Tutorial":
            tutorial_text = (
                f"# {emoji.fire} Tutorial de Uso\n\n"
                f"**Como resgatar o código:**\n"
                f"1. Faça login no site [Loop Gen](https://loopgen.vercel.app).\n"
