...
class ExtensionsPanel(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.bot_secret = os.getenv("LOOP_GEN_BOT_SECRET", "Loop-gen-bot-secret")
    ...
    def _build_components_panel(self, inter: disnake.MessageInteraction, extensions_config: dict) -> list:
        ...
        droxgen_data = self._get_droxgen_status()
        zurosgen_enabled = droxgen_data.get("enabled", True)
        zurosgen_integrated = droxgen_data.get("integrated_user_id") is not None

        # Construir status do Drox Gen
        if zurosgen_enabled and zurosgen_integrated:
            zurosgen_status = f"{emoji.on} Ativo"
            zurosgen_user = droxgen_data.get("integrated_user", "N/A")
        elif zurosgen_enabled:
            zurosgen_status = f"{emoji.warn} Não integrado"
            zurosgen_user = None
        else:
            zurosgen_status = f"{emoji.off} Desativado"
            zurosgen_user = None
        ...
        extension_options = [
            disnake.SelectOption(
                ...
                value="zurosgen",
                emoji=emoji.link,
                description=f"{'Ativo' if zurosgen_enabled else 'Não integrado'} - Gerador de contas"
            ),
            disnake.SelectOption(
                label="Zuros Boost",
                value="boost",
        ...
        # Drox Gen info
        droxgen_info = f"{emoji.link} **Drox Gen (Grátis)** — {zurosgen_status}"
        if zurosgen_user:
            droxgen_info += f"\n-# {emoji.member} Integrado com: `{zurosgen_user}`"
        else:
            droxgen_info += f"\n-# Gerador de contas com integração ao painel web"

        # Zuros Boost info
        boost_info = f"{emoji.boost} **Zuros Boost** — {boost_status}"
        ...
        return [
            disnake.ui.Container(
                ...
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
                disnake.ui.TextDisplay(droxgen_info),
                disnake.ui.Separator(spacing=disnake.SeparatorSpacing.small),
    ...
    def _build_embed_panel(self, inter: disnake.MessageInteraction, extensions_config: dict) -> tuple:
        ...
        # Drox Gen Field
        zurosgen_value = f"**Status:** {zurosgen_status}\n"
        if zurosgen_integrated:
            zurosgen_value += f"**Usuário:** `{droxgen_data.get('integrated_user', 'N/A')}`\n"
        zurosgen_value += "Gerador de contas com integração ao painel web."

        embed.add_field(
            name=f"{emoji.link} Drox Gen (Grátis)",
            value=zurosgen_value,
            inline=False
        )

        # Zuros Boost Field
        embed.add_field(
            name=f"{emoji.boost} Zuros Boost",
            value=(
    ...
    def _build_zurosgen_panel(self, inter: disnake.MessageInteraction) -> list:
        """Constrói o painel do Drox Gen"""
        colors = db.get_document("custom_colors")
        primary_color_hex = colors.get("primary")
        ...
        status_data = self._get_droxgen_status()
        ...
        # Status info
        status_text = f"# {emoji.zuros}\n-# Painel > Extensões > **Drox Gen**"
        ...
        return [
            ...
            disnake.ui.ActionRow(
                ...
                disnake.ui.Button(
                    ...
                    custom_id="DroxGen_Toggle"
                )
            )
        ]

    def _build_zurosgen_config_panel(self, inter: disnake.MessageInteraction) -> list:
        """Constrói o painel de configuração do Drox Gen"""
        colors = db.get_document("custom_colors")
        primary_color_hex = colors.get("primary")
        ...
        return [
            disnake.ui.Container(
                ...
                disnake.ui.ActionRow(
                    disnake.ui.StringSelect(
                        custom_id="DroxGen_ConfigSelect",
    ...
    async def on_extensions_button_click(self, inter: disnake.MessageInteraction):
        ...
        if custom_id == "Configuracoes_Extensoes":
            ...
            await inter.response.send_message(
                ...
                f"Todas as extensões são nomeadas pela **Zuros**, porém são oferecidas através de "
        ...
        elif custom_id == "Extensions_DroxGen":
            mode = db.get_document("custom_mode").get("mode")
            if mode == "embed":
                await embed_message.wait(inter, send=False)
                # For embed mode, we still use components V2
                components = self._build_zurosgen_panel(inter)
        ...
        elif custom_id == "DroxGen_BackToConfig":
            await message.wait(inter, send=False)
            components = self._build_zurosgen_config_panel(inter)
            await inter.edit_original_message(components=components)
        ...
        elif custom_id == "DroxGen_CancelLiveStock":
            await message.wait(inter, send=False)
            components = self._build_zurosgen_config_panel(inter)
            await inter.edit_original_message(components=components)

        elif custom_id == "DroxGen_LiveStockAnalytics":
            await self._show_livestock_analytics(inter)

        elif custom_id == "DroxGen_LiveStockNotifications":
            await self._toggle_livestock_notifications(inter)
        
        elif custom_id == "DroxGen_LiveStockProducts":
            await self._show_products_for_livestock(inter)
    ...
    async def _show_my_payments(self, inter: disnake.MessageInteraction):
        ...
        # Header
        container_items.append(disnake.ui.TextDisplay(f"# {emoji.zuros}\nMeus Pagamentos"))
    ...
    async def _show_purchase_panel(self, inter: disnake.MessageInteraction):
        ...
        purchase_select = disnake.ui.StringSelect(
            ...
            options=[
                disnake.SelectOption(
                    label="Drox Gen (Grátis)",
                    value="purchase_zurosgen",
        ...
        # Texto do Zuros Boost (com expiração se ativo)
        boost_title = f"{emoji.boost} **Zuros Boost (R$50 per/month)**"
        if is_active and expiry_date:
            boost_title = f"{emoji.boost} **Zuros Boost (Expira: {expiry_date})**"
    ...
    async def _create_boost_payment(self, inter: disnake.MessageInteraction):
        ...
        embed = disnake.Embed(
            ...
            description=(
                f"Realize o pagamento via PIX para ativar o **Zuros Boost**.\n\n"
    ...
    async def _verify_payment(self, inter: disnake.MessageInteraction, payment_id: str):
        ...
        if status == "COMPLETED":
            await inter.followup.send(
                ...
                f"A extensão **Zuros Boost** foi ativada com sucesso.\n"
    ...
    async def on_zurosgen_dropdown(self, inter: disnake.MessageInteraction):
        custom_id = inter.component.custom_id
        ...
        if custom_id == "Extensions_Purchase_Select":
            ...
            if choice == "purchase_zurosgen":
                 await inter.response.send_message(
                     f"{emoji.link} **Drox Gen** é uma extensão gratuita e já está disponível para uso.",
                     ephemeral=True
                 )
        ...
        if custom_id == "Extensions_Select":
            ...
            if choice == "zurosgen":
                ...
                if boost_cog:
                    ...
                    await inter.response.send_message(
                        f"{emoji.warn} Extensão Zuros Boost não está disponível.",
                        ephemeral=True
                    )
        ...
        if custom_id == "DroxGen_MainSelect":
            choice = inter.values[0]
            ...
            if choice == "generate_code":
                ...
                tutorial_text = (
                    ...
                    f"**Passo 2:** Acesse o **Dashboard** e clique em **Integrar Drox Bot**\n\n"
                    f"**Passo 3:** No bot, clique em **Gerar Código** no menu acima\n\n"
                    f"**Passo 4:** Copie o código gerado e cole no modal do site (que apareceu quando você clicou em Integrar Drox Bot)\n\n"
        ...
        elif custom_id == "DroxGen_ConfigSelect":
            choice = inter.values[0]
        ...
        elif custom_id == "DroxGen_ProjectSelect":
            val = inter.values[0]
        ...
        elif custom_id == "DroxGen_LiveStockProduct":
                    ...
                    if response.status == 200:
                        ...
                        components = [
                            disnake.ui.Container(
                                ...
                                disnake.ui.ActionRow(
                                    disnake.ui.StringSelect(
                                        custom_id="DroxGen_ProjectSelect",
            ...
            components = [
                ...
                disnake.ui.ActionRow(
                    disnake.ui.Button(
                        ...
                        style=disnake.ButtonStyle.green,
                        emoji=emoji.correct,
                        custom_id="DroxGen_ConfirmLiveStock"
                    ),
                    disnake.ui.Button(
                        label="Cancelar",
                        style=disnake.ButtonStyle.grey,
                        emoji=emoji.wrong,
                        custom_id="DroxGen_CancelLiveStock"
                    )
                )
            ]
    ...
    async def _show_products_for_livestock(self, inter: disnake.MessageInteraction):
        ...
        for product_id, p in products_dict.items():
            ...
            if product_name:
                ...
                droxgen_data = self._get_droxgen_status()
                livestock = droxgen_data.get("livestock", {})
        ...
        components = [
            disnake.ui.Container(
                ...
                disnake.ui.ActionRow(
                    disnake.ui.StringSelect(
                        custom_id="DroxGen_LiveStockProduct",
            ...
            disnake.ui.ActionRow(
                disnake.ui.Button(
                    ...
                    style=disnake.ButtonStyle.grey,
                    emoji=emoji.back,
                    custom_id="DroxGen_BackToConfig"
                ),
                disnake.ui.Button(
                    label="Análise",
                    style=disnake.ButtonStyle.blurple,
                    emoji=emoji.chart,
                    custom_id="DroxGen_LiveStockAnalytics"
                ),
                disnake.ui.Button(
                    label="Notificações",
                    style=disnake.ButtonStyle.grey,
                    emoji=emoji.announcement if hasattr(emoji, 'announcement') else emoji.message,
                    custom_id="DroxGen_LiveStockNotifications"
                )
            )
        ]
        ...
        components = [
            ...
            disnake.ui.ActionRow(
                disnake.ui.Button(
                    ...
                    style=disnake.ButtonStyle.grey,
                    emoji=emoji.back,
                    custom_id="DroxGen_LiveStockProducts"
                )
            )
        ]
