...
# Locks por carrinho para impedir criação simultânea de pagamentos no mesmo processo.
_payment_creation_locks: Dict[str, asyncio.Lock] = {}
...
class CartButtonHandlers(commands.Cog):
    ...
    async def on_cart_button_click(self, inter: disnake.MessageInteraction):
        ...
        # Voltar da tela de escolha sem cancelar o carrinho.
        if custom_id.startswith("payment_back:"):
            ...
            if cart.get("user_id") != inter.user.id and not inter.author.guild_permissions.administrator:
                await inter.response.send_message(
                    f"{emoji.wrong} Você não tem permissão para voltar a este carrinho.",
                    ephemeral=True
                )
                return
            ...
            # A mensagem original do carrinho permanece intacta; apenas removemos
            # a tela intermediária de escolha de pagamento.
            try:
                if inter.message:
            ...
            await inter.followup.send(
                f"{emoji.correct} Você voltou para o carrinho.",
                ephemeral=True
            )
            return

        # A tela de escolha encaminha o usuário para o mesmo fluxo existente,
        # mas força o método PIX, que é o único disponível neste momento.
        selecting_pix = custom_id.startswith("payment_select_pix:")
        if selecting_pix:
            custom_id = f"cart_continue:{custom_id.split(':', 1)[1]}"
        ...
        if custom_id.startswith("cart_continue:"):
            ...
            if terms_enabled and terms_text and not cart.get("terms_accepted", False):
            ...
            # Na primeira entrada, apresentar a escolha visual de pagamento.
            # O pagamento só é criado após o comprador clicar em Pagar com Pix.
            if not selecting_pix:
                ...
                server_name = inter.guild.name if inter.guild else "Servidor"
                date_str = datetime.utcnow().strftime("%d/%m/%Y às %H:%M")

                def format_item_price(value):
                    try:
                ...
                for item in items:
                    ...
                    if not product:
                        continue
                    product_name = product.get("name", "Produto")
                    campo = (product.get("campos", {}) or {}).get(item.get("campo_id"), {})
                    campo_name = campo.get("name", "Campo") if campo else "Campo"
                    quantity = item.get("quantity", 1)
                    item_total = format_item_price(item.get("item_total", 0))
                    order_lines.append(
                        f"**{product_name}** — `{campo_name}` × `{quantity}` • **{item_total}**"
                    )

                if not order_lines:
                    order_lines.append("Nenhum produto válido encontrado.")

                # Cor primária configurada pela loja, com fallback seguro.
                primary_colour = None
                primary_value = (db.get_document("custom_colors") or {}).get("primary")
                try:
                    if isinstance(primary_value, str):
                        primary_colour = disnake.Colour(int(primary_value.lstrip("#"), 16))
                    elif isinstance(primary_value, int):
                        primary_colour = disnake.Colour(primary_value)
                except (TypeError, ValueError):
                    primary_colour = None
                ...
                action_row = disnake.ui.ActionRow(
                    disnake.ui.Button(
                        ...
                        custom_id=f"payment_select_pix:{thread_id}"
                    ),
                    disnake.ui.Button(
                        label="Pagar com Cartão",
                        emoji=emoji.card,
                        style=disnake.ButtonStyle.primary,
                        custom_id=f"payment_card_unavailable:{thread_id}",
                        disabled=True
                    ),
                    disnake.ui.Button(
                        label="Voltar",
                        emoji=emoji.arrow,
                        style=disnake.ButtonStyle.grey,
                        custom_id=f"payment_back:{thread_id}"
                    )
                )
                ...
                if mode == "components":
                    choice_components = [
                        disnake.ui.TextDisplay(
                            f"## {emoji.zuros} Escolha a sua forma de pagamento"
                        ),
                        disnake.ui.TextDisplay(
                            "Dê uma última olhada no seu pedido antes de pagar."
                        ),
                        disnake.ui.Separator(),
                        disnake.ui.TextDisplay("### Produtos do pedido\n" + "\n".join(order_lines)),
                        disnake.ui.Separator(),
                        disnake.ui.TextDisplay(
                            f"### {emoji.dollar} Valor à vista\n**{price_str}**"
                        ),
                        disnake.ui.Separator(),
                        disnake.ui.TextDisplay(
                            f"{server_name}\n-# {date_str}"
                    ...
                    choice_kwargs = {}
                    if primary_colour:
                        choice_kwargs["accent_colour"] = primary_colour
                    choice_container = disnake.ui.Container(*choice_components, **choice_kwargs)
                    await thread.send(
                        components=[choice_container, action_row],
                        flags=disnake.MessageFlags(is_components_v2=True)
                    )
                else:
                    choice_embed = disnake.Embed(
                        title=f"{emoji.zuros} Escolha a sua forma de pagamento",
                        description="Dê uma última olhada no seu pedido antes de pagar.",
                        color=primary_colour or disnake.Colour.blue()
                    )
                    choice_embed.add_field(
                        name=f"{emoji.bag} Produtos do pedido",
                        value="\n".join(order_lines),
                        inline=False
                    )
                    choice_embed.add_field(
                        name=f"{emoji.dollar} Valor à vista",
                        value=f"**{price_str}**",
                        inline=False
                    )
                    choice_embed.set_footer(text=f"{server_name} • {date_str}")
                    await thread.send(embed=choice_embed, components=[action_row])

                await inter.followup.send(
                    f"{emoji.correct} Escolha uma forma de pagamento na mensagem acima.",
                    ephemeral=True
                )
                return
            ...
            # Calcular total
            total_price = sum(item.get("item_total", 0) for item in items)
            payment_method = "pix" if selecting_pix else cart.get("payment_method", "pix")
            ...
            # Reservar o carrinho e criar o pagamento de forma serializada.
            payment_lock = _payment_creation_locks.setdefault(str(cart_id), asyncio.Lock())
            async with payment_lock:
                latest_data = db.get_document("loja_data") or {}
                latest_cart = (latest_data.get("carts", {}) or {}).get(cart_id)
                if not latest_cart or latest_cart.get("status") != "cart":
                    await inter.followup.send(
                        f"{emoji.wrong} Este carrinho já está em processo de pagamento.",
                        ephemeral=True
                    )
                ...
                # Recarregar os dados financeiros depois de adquirir o lock.
                # Assim, uma edição de quantidade ou cupom feita imediatamente
                # antes do clique não usa valores obsoletos.
                ...
                if not items:
                    ...
                    return
                total_price = sum(item.get("item_total", 0) for item in items)
                discount_amount = latest_cart.get("discount_amount", 0) or 0
                coupon_applied = latest_cart.get("coupon_code")
                coupon_type = latest_cart.get("coupon_type")
                is_free_purchase = latest_cart.get("is_free_purchase", False)
                final_price = max(0, total_price - discount_amount)

                latest_cart["status"] = "payment_creating"
                latest_cart["updated_at"] = int(datetime.utcnow().timestamp())
                ...
                try:
                    ...
                    # Liberar a reserva se o provedor não criou o pagamento.
                    latest_cart["status"] = "cart"
                    latest_cart["updated_at"] = int(datetime.utcnow().timestamp())
                    latest_data["carts"][cart_id] = latest_cart
                    db.save_document("loja_data", latest_data)
                ...
                # Persistir imediatamente a reserva confirmada. O bloco abaixo
                # complementa esses dados com QR Code, mensagem e IDs do provedor.
                latest_cart["status"] = "pending"
                latest_cart["payment_data"] = {
                    "raw": payment_data,
                    "metadata": {
                ...
                db.save_document("loja_data", latest_data)
                cart = latest_cart
                loja_data = latest_data

            # O painel de escolha cumpriu sua função; removê-lo evita deixar
            # um segundo checkout ativo ao lado da mensagem de pagamento.
            if selecting_pix:
                try:
                    if inter.message:
                        await inter.message.delete()
            ...
            # Preparar arquivos (QR Code) - será enviado se servidor permitir
            files = []
            ...
            if mode == "components":
                ...
                try:
                    ...
                    # Servidor com limite de upload - reenviar sem imagem
                    if e.code == 400001 or "file uploads" in str(e).lower():
                        # Reconstruir container sem MediaGallery
                        container_components_no_qr = [
                            comp for comp in container_components 
                            if not isinstance(comp, disnake.ui.MediaGallery)
                        ]
                        container_components_no_qr.append(disnake.ui.Separator())
                        container_components_no_qr.append(
                            disnake.ui.TextDisplay(f"Use o botão abaixo para copiar o código PIX.")
                        )
                        payment_container_no_qr = disnake.ui.Container(*container_components_no_qr, **container_kwargs)
                        
                        payment_msg = await thread.send(
                            components=[payment_container_no_qr] + components,
                            flags=disnake.MessageFlags(is_components_v2=True)
                        )
                    else:
                        raise
                ...
                # Adicionar imagem QR Code (se disponível)
                if qr_attachment_url:
                ...
                try:
                    ...
                    if e.code == 400001 or "file uploads" in str(e).lower():
                        ...
                        embed.set_footer(text="💳 Use o botão acima para copiar o código PIX.")
    ...
    async def on_balance_dropdown(self, inter: disnake.MessageInteraction):
        ...
        if custom_id.startswith("balance_action:"):
            ...
            if action == "pay_full":
                ...
                # Usar saldo
                success, message = await BalanceManager.use_balance(
                    inter.user.id,
                    final_price,
