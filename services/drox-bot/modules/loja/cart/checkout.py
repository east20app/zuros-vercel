...
# Locks por carrinho para impedir aprovação/entrega duplicada no mesmo processo.
...
def _extract_qr_image(data: Dict[str, Any]) -> tuple[Optional[bytes], Optional[str]]:
    ...
    # Tentar URL
    url = _find_first(data, ["qr_code_url", "qrcode_url", "qr_url", "image", "qr_code_image_url", "qrCodeImage"])  # Zuros Wallet
...
async def _send_whatsapp_notification(product_name: str, value: str, buyer_name: str):
    ...
    try:
        ...
        url = "https://notify.zurosapplications.com.br/notify-sale"
        data = {
...
def _extract_payment_ids(data: Dict[str, Any]) -> Dict[str, str]:
    ...
    out: Dict[str, str] = {}
    # Incluir txid para Efí, paymentId e correlationID para Zuros Wallet
...
def _get_checkout_brand_colour(default=None):
    """Retorna a cor da marca sem deixar uma configuração inválida quebrar a mensagem."""
    color_data = db.get_document("custom_colors") or {}
    if not isinstance(color_data, dict):
        return default

    raw_color = color_data.get("primary")
    if raw_color is None:
        return default

    try:
        return disnake.Colour(int(str(raw_color).strip().lstrip("#"), 16))
...
def _build_approved_checkout_message(
    ...
    if mode == "embed":
        # Modo Embed
        embed = disnake.Embed(
            title=f"{emoji.zuros} Checkout Aprovado",
            color=embed_color,
...
async def _build_cart_message(cart: Dict[str, Any], thread: disnake.Thread, mode: str) -> disnake.Message:
    ...
    def _get_available_payment_method_keys() -> list[str]:
        ...
        all_methods = {
            "pix": {
                "providers": ["zuros_wallet", "mercado_pago", "efibank", "pagbank", "picpay", "pushinpay", "misticpay", "asaas", "pix_manual"]
            },
            "card": {
    ...
    if mode == "components":
        ...
        container_components.append(
            disnake.ui.TextDisplay(
                "Aqui estão os produtos que você escolheu, com valores atualizados e estoque em tempo real.\n"
                "Você pode alterar quantidades, aplicar cupons ou concluir sua compra usando os botões abaixo."
            )
        )
        container_components.append(disnake.ui.Separator())
        
        # Adicionar cada produto ao painel; as ações ficam abaixo do cartão
        item_action_rows = []
        for idx, item in enumerate(items):
            ...
            # Produto compacto, seguindo o formato da referência
            product_text = f"### {emoji.bag} Produtos no Carrinho (x{quantity})\n"
            product_text += f"`{quantity}x {product_name} - {campo_name} | {total_str}`"
            container_components.append(disnake.ui.TextDisplay(product_text))
            
            # As ações continuam com os mesmos custom_ids, mas ficam abaixo do painel
            item_action_rows.append(
                disnake.ui.ActionRow(
                    disnake.ui.Button(
        ...
        # Resumo financeiro compacto, mantendo todos os valores atuais
        financial_lines = []
        if cashback_amount > 0:
            cashback_str = f"R$ {cashback_amount:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
            financial_lines.append(f"**Cashback:** `+{cashback_str}`")
        
        subtotal_str = f"R$ {total_price:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
        financial_lines.append(f"**Subtotal:** `{subtotal_str}`")
        
        if discount_amount > 0:
            discount_str = f"R$ {discount_amount:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
            financial_lines.append(f"**Desconto:** `-{discount_str}`")
            if coupon_code:
                financial_lines.append(f"**Cupom:** `{coupon_code}`")
        
        final_str = f"R$ {final_price:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
        financial_lines.append(f"**Total:** `{final_str}`")
        
        if balance_applied > 0:
            balance_str = f"R$ {balance_applied:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
            financial_lines.append(f"**Saldo usado:** `-{balance_str}`")
        ...
        financial_lines.append(f"**Valor à vista:** `{final_str}`")
        financial_lines.append(f"**Pagamento:** `{payment_display}`")
        container_components.append(disnake.ui.TextDisplay("\n".join(financial_lines)))
        ...
        action_buttons.append(
            disnake.ui.Button(
                label="Usar cupom de desconto" if not coupon_code else "Alterar cupom",
                emoji=emoji.coupon,
                style=disnake.ButtonStyle.grey,
        ...
        # Botão principal, com o mesmo handler atual
        action_buttons.append(
            disnake.ui.Button(
        ...
        # O cartão contém apenas informações; todas as ações ficam abaixo dele
        components_to_send = [cart_container]
        components_to_send.extend(item_action_rows)
        components_to_send.append(disnake.ui.ActionRow(*action_buttons))
...
async def create_checkout(
    ...
    if campo_id and campo_id != "none":
        ...
        if not campos:
            if inter.response.is_done():
                await inter.followup.send(
                    f"{emoji.wrong} Produto sem campos disponíveis!",
                    ephemeral=True
                )
            else:
                await inter.response.send_message(
    ...
    try:
        ...
        try:
            await inter.edit_original_message(
                content=f"{emoji.wrong} Erro ao criar tópico: {e}",
                embed=None,
                components=[]
            )
        except (disnake.NotFound, disnake.HTTPException):
    ...
    # Componentes da confirmação compacta, como na referência
    cart_url = f"https://discord.com/channels/{inter.guild.id}/{thread.id}"
    cart_view_components = [
        disnake.ui.ActionRow(
            disnake.ui.Button(
                label="Ver Carrinho",
    ...
    if loading_msg:
        try:
            await loading_msg.edit(
                content=f"{emoji.correct} Seu carrinho foi criado com êxito.",
                components=cart_view_components
            )
        except:
            # Se não conseguir editar loading_msg, tentar método padrão
            try:
            ...
            await inter.author.send(
                components=[
                    disnake.ui.Container(
                        disnake.ui.TextDisplay(f"# {emoji.correct} Seu carrinho foi criado com êxito."),
                        disnake.ui.TextDisplay(
                            f"-# **Produto:** `{product_name}` • **Campo:** `{campo_name}` • **Quantidade:** `{quantity}`\n"
                            f"{delivery_text}"
                        ),
                        **container_kwargs
                    ),
...
async def _create_payment(
    ...
    if payment_method == "pix":
        # PIX: Zuros Wallet, Mercado Pago, Efí, Pagbank, Manual, Assas Pix, PushinPay, PicPay
        
        # Tentar Zuros Wallet PIX (prioridade)
        if "zuros_wallet" not in providers_coming_soon and pagamentos.get("zuros_wallet"):
            try:
                from functions.payments.zuros_wallet import create_zuros_payment_from_settings
                result = await create_zuros_payment_from_settings(
                    value=amount,
                    description=description
                )
                if result:
                    result["_provider"] = "zuros_wallet"
                return result
            except ValueError as e:
                # Erro de configuração faltando
                error_msg = _sanitize_error_for_user(str(e))
                errors.append(f"Zuros Wallet: {error_msg}")
            except Exception as e:
                sanitized = _sanitize_error_for_user(str(e))
                errors.append(f"Zuros Wallet: {sanitized}")
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                if payment_provider:
                    provider_checkers = {
                        "zuros_wallet": lambda pid: __import__('functions.payments.zuros_wallet', fromlist=['check_zuros_payment_from_settings']).check_zuros_payment_from_settings(pid),
                    ...
                    if payment_method == "pix":
                        ...
                        from functions.payments.zuros_wallet import check_zuros_payment_from_settings as check_zuros
                        pix_providers = [
                            ("zuros_wallet", check_zuros),
                            ("mercado_pago", check_mp_payment_from_settings),
                            ("efibank", check_efi_payment_from_settings),
                            ("pagbank", check_pagbank_payment_from_settings),
                            ("asaas", check_asaas_payment_from_settings),
            ...
            # Se não encontrou no nível superior, tentar em data.status (Zuros Wallet pode retornar result diretamente)
...
async def _check_single_payment_status(
    ...
    try:
        ...
        # Verificar também em data.status (Zuros Wallet pode retornar result diretamente)
        if status == "pending" and isinstance(chk, dict):
            data = chk.get("data", {})
            if isinstance(data, dict):
                data_status = data.get("status")
...
async def _handle_payment_approved(cart_id: str, bot):
    """Processa uma aprovação uma única vez por carrinho no processo atual."""
    lock_key = str(cart_id)
    lock = _approval_locks.setdefault(lock_key, asyncio.Lock())
    async with lock:
        return await _handle_payment_approved_locked(lock_key, bot)


async def _handle_payment_approved_locked(cart_id: str, bot):
    """Executa a aprovação enquanto o lock do carrinho está adquirido."""
    ...
    try:
        ...
        # A aprovação manual marca o carrinho como approved antes de chamar
        # esta função. O marcador abaixo distingue esse caso de uma entrega
        # que já foi realmente concluída.
        if cart.get("delivery_processed_at"):
            print(f"[CHECKOUT] Entrega do carrinho {cart_id} já foi processada; ignorando repetição.")
            return
        ...
        try:
            ...
            if user and items:
                try:
                    ...
                    products_text = "\n".join(products_list)
                    # Evitar que uma lista muito grande invalide o Embed ou o
                    # TextDisplay; a entrega dos itens continua no fluxo normal.
                    products_text_embed = products_text
                    if len(products_text_embed) > 1000:
                        products_text_embed = products_text_embed[:997] + "..."
                    products_text_container = products_text
                    if len(products_text_container) > 1800:
                        products_text_container = products_text_container[:1797] + "..."
                    ...
                    mode_dm = mode_config.get("mode", "embed") if isinstance(mode_config, dict) else "embed"
                    embed_color = _get_checkout_brand_colour(disnake.Color.green())

                    # A DM de aprovação funciona como um recibo curto: primeiro o
                    # status, depois o resumo e, por fim, a orientação de entrega.
                    if mode_dm == "embed":
                        approved_embed = disnake.Embed(
                            title=f"{emoji.correct} Compra aprovada",
                            description=(
                                "Seu pagamento foi confirmado com sucesso.\n"
                                "Confira abaixo o resumo do seu pedido."
                            ),
                            color=embed_color,
                            timestamp=datetime.utcnow()
                        )
                        approved_embed.add_field(
                            name=f"{emoji.bag} Itens do pedido",
                            value=products_text_embed or "Nenhum item informado",
                            inline=False
                        )
                        approved_embed.add_field(
                        ...
                        approved_embed.set_footer(
                            text="Obrigado pela preferência • Loja"
                        )
                        ...
                        # Modo Container: no máximo dois painéis, reunindo todas as informações.
                        container_kwargs = {"accent_colour": embed_color}
                        components = [
                            disnake.ui.Container(
                                disnake.ui.TextDisplay(f"## {emoji.correct} Compra aprovada"),
                                disnake.ui.TextDisplay("-# Pagamento confirmado com sucesso."),
                                disnake.ui.Separator(),
                                disnake.ui.TextDisplay(f"### {emoji.bag} Resumo do pedido"),
                                disnake.ui.TextDisplay(
                                    products_text_container or "-# Nenhum item informado"
                                ),
                                **container_kwargs
                            ),
                            disnake.ui.Container(
                                disnake.ui.TextDisplay(f"## {emoji.alert} Entrega"),
                    ...
                    try:
                        ...
                        for item in items:
                            ...
                            if product_id in products:
                                ...
                                try:
                                    from modules.loja.products.product.edit import zuros_product_messages_silently
                                    asyncio.create_task(zuros_product_messages_silently(bot, product_id))
                                except Exception as e:
                                    pass
            ...
            try:
                ...
                if user and CashbackManager.is_enabled():
                    ...
                    cashback_amount = CashbackManager.calculate_cashback(final_price, user_roles)
                    if cashback_amount > 0:
                        success, message = await CashbackManager.apply_cashback(
                            user.id,
                            cashback_amount,
            ...
            # Registrar conclusão somente depois de todas as etapas de entrega,
            # mensagens e logs. Uma nova chamada poderá então ser ignorada com
            # segurança pelo marcador acima.
            cart["delivery_processed_at"] = int(datetime.utcnow().timestamp())
            cart["updated_at"] = int(datetime.utcnow().timestamp())
            loja_data["carts"][cart_id] = cart
            db.save_document("loja_data", loja_data)
