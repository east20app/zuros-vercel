    ...
    async def _handle_copy_content(self, inter: disnake.MessageInteraction):
        ...
        if inter.message.embeds:
            ...
            for field in embed.fields:
                ...
                if "seus itens" in field.name.casefold() or "itens" in field.name.casefold():
        ...
        if not content_text and inter.message.components:
            try:
                ...
                def parse_items_text(text):
                    """Extrai o conteúdo após o título `Seus itens`, sem depender de capitalização."""
                    if not text:
                        return None
                    ...
                    for line in lines:
                        ...
                        lower_line = line_clean.casefold()
                        marker_index = lower_line.find(marker)

                        if marker_index >= 0:
                            found_items = True
                            remaining = line_clean[marker_index + len(marker):]
                ...
                for idx, component_row in enumerate(inter.message.components):
                    ...
                    if is_container:
                        ...
                        if children:
                            ...
                            # O título e os itens normalmente são TextDisplays
                            # separados; unir o conteúdo do Container permite
                            # localizar corretamente o bloco exibido ao usuário.
                            if not content_text:
                                block_texts = []
                                for child in children:
                                    child_text = extract_text_from_textdisplay(child)
                                    if child_text:
                                        block_texts.append(child_text)
                                if block_texts:
                                    content_text = parse_items_text("\n".join(block_texts))
                        else:
                                            ...
                                            if hasattr(component_row, 'children'):
                                                for item in component_row.children:
                                                    if isinstance(item, disnake.ui.TextDisplay):
                                                        ...
                                                        if text and "seus itens" in text.casefold():
