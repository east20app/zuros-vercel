...
async def send_payment_approved_dm(
    ...
    try:
        ...
        if mode == "embed":
            ...
            embed.add_field(
                name=f"{emoji.bag if hasattr(emoji, 'bag') else '🛍️'} Resumo do pedido",
                value=(
                    f"**Produto:** `{product_name}`\n"
                    f"**Opção:** `{campo_name}`\n"
                    f"**Quantidade:** `{quantity}`"
                ...
                components = [
                    disnake.ui.ActionRow(
                        disnake.ui.Button(
                            ...
                            url=thread_url,
                            emoji=emoji.cart if hasattr(emoji, 'cart') else None
                        )
                    )
                ]
            ...
            container_items = [
                ...
                disnake.ui.Separator(),
                disnake.ui.TextDisplay(
                    f"### {emoji.bag if hasattr(emoji, 'bag') else '🛍️'} Resumo do pedido\n"
                    f"-# **Produto:** `{product_name}`\n"
                    f"-# **Opção:** `{campo_name}`\n"
                    f"-# **Quantidade:** `{quantity}`"
                ),
