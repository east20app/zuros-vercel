...
class CancelCheckout(commands.Cog):
    ...
    async def on_close_cart_button(self, inter: disnake.MessageInteraction):
        ...
        if custom_id.startswith("close_cart:"):
            ...
            await inter.response.send_message(
                f"{emoji.loading} Encerrando atendimento...",
                ephemeral=True
            )
            
            try:
                # Cancelar pagamento na Zuros Pay API se aplicável e ainda não aprovado
                if cart.get("status") not in ["approved", "cancelled"]:
                    ...
                    if payment_provider == "zuros_wallet":
                        try:
                            from functions.payments.zuros_wallet import cancel_zuros_payment_from_settings
                            ...
                            if payment_id:
                                # Cancelar na Zuros Pay API
                                result = await cancel_zuros_payment_from_settings(payment_id)
                                print(f"✅ Pagamento Zuros Wallet {payment_id} cancelado ao encerrar: {result}")
                            else:
                                print(f"⚠️ Payment ID não encontrado para cancelamento Zuros Wallet")
                                
                        except Exception as e:
                            # Não falhar o encerramento se houver erro ao cancelar na API
                            print(f"❌ Erro ao cancelar pagamento Zuros Wallet: {e}")
    ...
    async def on_cancel_button(self, inter: disnake.MessageInteraction):
        ...
        if custom_id.startswith("cancel_checkout:"):
            ...
            if not (is_owner or is_admin):
                await inter.response.send_message(
                    f"{emoji.wrong} Você não tem permissão para cancelar este checkout!",
                    ephemeral=True
                )
                return
            ...
            await inter.response.send_message(
                f"{emoji.loading} Cancelando checkout...",
                ephemeral=True
            )
            
            # Cancelar pagamento na Zuros Pay API se aplicável
            payment_data = cart.get("payment_data", {})
            ...
            if payment_provider == "zuros_wallet":
                try:
                    ...
                    if payment_id:
                        # Cancelar na Zuros Pay API
                        result = await cancel_zuros_payment_from_settings(payment_id)
                        print(f"✅ Pagamento Zuros Wallet {payment_id} cancelado: {result}")
                    else:
                        print(f"⚠️ Payment ID não encontrado para cancelamento Zuros Wallet")
            ...
            # Avisar o comprador no mesmo padrão visual da referência.
            # A lógica de cancelamento, devolução e exclusão do tópico permanece igual.
            try:
                user = await self.bot.fetch_user(cart.get("user_id"))
                if user:
                    cancellation_text = (
                        "Olá! A sua compra foi cancelada por inatividade, "
                        "e todos os produtos foram devolvidos para o estoque. "
                        "Você pode voltar a comprar quando quiser!"
                    )
                    ...
                    if mode == "embed":
                        ...
                        cancelled_container = disnake.ui.Container(
                            disnake.ui.TextDisplay("## Compra Cancelada"),
                            disnake.ui.TextDisplay(cancellation_text),
                            accent_colour=disnake.Colour.red()
                        )
                        await user.send(
                            components=[cancelled_container],
                            flags=disnake.MessageFlags(is_components_v2=True)
                        )
            except Exception as notify_error:
                # A falha na DM não pode impedir o cancelamento nem a remoção do carrinho.
                print(f"Aviso: não foi possível notificar o comprador sobre o cancelamento: {notify_error}")
            ...
            try:
                ...
                try:
                    if not inter.response.is_done():
                        await inter.response.send_message(
                            content=f"{emoji.wrong} Erro ao processar cancelamento: {e}",
                            ephemeral=True
                        )
                    else:
                        try:
    ...
    async def on_copy_pix_button(self, inter: disnake.MessageInteraction):
        ...
        if custom_id.startswith("copy_pix:"):
            ...
            # Tentar nova estrutura primeiro
            local_data = payment_data.get("local", {}) if payment_data else {}
            ...
            if not copy_code:
                await inter.response.send_message(
                    f"{emoji.wrong} Código PIX não disponível!",
                    ephemeral=True
                )
                return
