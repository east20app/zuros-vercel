...
def _get_http_base_url() -> str:
    """Obtém a URL base para chamadas HTTP da Zuros Cloud API.
    ...
    try:
        ...
        return (
            ...
            or "https://cloud.zurosapplications.com.br"
        )
    except Exception:
        return "https://cloud.zurosapplications.com.br"
...
async def initialize_websocket():
    ...
    try:
        ...
        async def on_message(message_data):
            ...
            if event == 'auth_log':
                logger.debug('auth_log legado ignorado; Zuros Auth usa REST/polling')
                ...
                logger.debug('remove_verified_role legado ignorado; cargos usam role-sync REST')
...
async def _give_verified_role_internal(bot, auth_data: dict):
    ...
    try:
        ...
        if not already_has_role:
            await member.add_roles(verified_role, reason="Verificação automática via ZurosCloud")
        ...
        if definitions.get("remove_autorole", {}).get("enabled", False):
            ...
            if not autorole_id:
                ...
                if not autorole:
                    ...
                    try:
                        await member.remove_roles(autorole, reason="Remoção de autorole após verificação via ZurosCloud")
...
async def _remove_verified_role_internal(bot, auth_data: dict):
    ...
    try:
        ...
        # Remover o cargo
        await member.remove_roles(verified_role, reason="Desverificação automática via ZurosCloud")
