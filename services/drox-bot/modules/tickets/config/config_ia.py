...
class EditIAPromptModal(disnake.ui.Modal):
    def __init__(self, panel_id: str):
        ...
        super().__init__(title="Editar Instruções da DroxAI", components=components)
...
def ConfigIAView_components(inter: disnake.Interaction, panel_id: str) -> list:
    ...
    header_text = f"# {emoji.zuros}\n-# Gerenciar Tickets > Editar Painel > {panel_name} > **DroxAI**"
    status_text = (
...
def ConfigIAView_embed(inter: disnake.Interaction, panel_id: str):
    ...
    embed = disnake.Embed(
        title=f"Configurar DroxAI: {panel_data.get('name')}",
