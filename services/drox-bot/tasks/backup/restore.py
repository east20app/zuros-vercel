...
from functions.emoji import emoji as drox_emoji
import hashlib
        ...
        if "roles" in tipos_set:
            for role in list(guild.roles):
                if not role.is_default():
                    try:
                        await role.delete(reason="[Drox] Backup restore - wipe roles")
                    except Exception:
                        pass
            created_roles = {}
            bot_member = guild.me
        ...
        if "members" in tipos_set and data.get('members'):
            ...
            for member_data in data.get('members', []):
                try:
                    ...
                    if roles_to_add:
                        await member.add_roles(*roles_to_add, reason="[Drox] Backup restore - members")
            ...
            try:
                ...
                if user:
                    ...
                    if mode == "embed":
                        ...
                        embed = disnake.Embed(
                            title=f"{drox_emoji.zuros} Backup Restaurado com Sucesso!",
                        ...
                        embed.add_field(
                            ...
                            value=f"{drox_emoji.textc} **Canais:** `{get_count('channels')}`\n"
                                  f"{drox_emoji.dir} **Categorias:** `{get_count('categories')}`\n"
                                  f"{drox_emoji.role} **Cargos:** `{get_count('roles')}`\n"
                                  f"{drox_emoji.members} **Membros:** `{get_count('members')}`\n"
                                  f"{drox_emoji.reaction} **Emojis:** `{get_count('emojis')}`\n"
                                  f"{drox_emoji.flag} **Figurinhas:** `{get_count('stickers')}`\n"
                                  f"{drox_emoji.message if hasattr(drox_emoji, 'message') else '💬'} **Mensagens:** `{get_count('messages')}`\n"
                        )
                        ...
                        await dm.send(
                            components=[
                                disnake.ui.Container(
                                    ...
                                    disnake.ui.TextDisplay(
                                        f"{drox_emoji.textc} **Canais:** `{get_count('channels')}`\n"
                                        f"{drox_emoji.dir} **Categorias:** `{get_count('categories')}`\n"
                                        f"{drox_emoji.role} **Cargos:** `{get_count('roles')}`\n"
                                        f"{drox_emoji.members} **Membros:** `{get_count('members')}`\n"
                                        f"{drox_emoji.reaction} **Emojis:** `{get_count('emojis')}`\n"
                                    ...
                                    disnake.ui.Separator(),
                                    disnake.ui.TextDisplay(
                                        f"{drox_emoji.calendar} **Data:** <t:{agora}:f> (<t:{agora}:R>)\n"
                                    ),
                                )
                            ],
