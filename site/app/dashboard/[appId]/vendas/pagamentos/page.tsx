...
export async function generateMetadata({ params }: { params: Promise<{ appId: string }> }): Promise<import("next").Metadata> { const { appId } = await params; return vendasMetadata(appId, "Recebimento / PIX", (botName) => `Configuração de recebimento do bot ${botName}.`); }
        ...
        <main className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
            ...
            <div className="sales-chart-wrap">
                ...
                <div className="grid gap-3">
                    <div className="max-w-md"><Card className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-lg bg-emerald-500/10 text-emerald-300"><Icon name="payment" /></span><div><b className="text-sm text-white">PIX</b><p className="text-xs text-zinc-500">Forma de pagamento disponível na versão atual do Zuros Bot.</p></div></Card></div>
