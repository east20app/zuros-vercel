...
export default async function VendasPage({ params }: { params: Promise<{ appId: string }> }) {
    ...
    return <main className="sales-page mx-auto max-w-6xl px-5 py-8">
        <section className="sales-hero"><div><p className="home-kicker"><span className="home-kicker-mark" />02 / PERFORMANCE</p><h1>Visão geral.</h1><p>Acompanhe vendas, conversão e a saúde do bot <strong>{ctx.botName}</strong>.</p></div><div className="sales-hero-period"><span>JANELA ATIVA</span><strong>Últimos 7 dias</strong><small>Atualizado agora</small></div></section>
        <section className="sales-status-strip" aria-label="Status da operação"><div className="sales-status-main"><span className="sales-status-dot" /><div><strong>{ctx.botName} disponível</strong><small>{ctx.productName} · dados sincronizados para esta conta</small></div></div><span className="sales-status-chip"><i /> operação monitorada</span></section>
