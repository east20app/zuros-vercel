...
export async function GET(request: Request, context: Context) {
    ...
    try {
        ...
        if (route === "stats") { const filter = userId ? { ownerId: userId } : {}; const apps = await databases.applications.find(filter, { _id: 1, ownerId: 1, status: 1 }).lean(); const checks = await Promise.allSettled(apps.filter((app) => app.status === "active").map((app) => applicationOnline(String(app._id), String(app.ownerId)))); const online = checks.filter((result) => result.status === "fulfilled" && result.value).length; return respond({ applications: apps.length, active: apps.filter((app) => app.status === "active").length, online }, id); }
