...
export function DashboardAppsGrid({ apps }: { apps: AppSummary[] }) {
    ...
    return <>
        ...
        {visible.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{visible.map((app, index) => <DashboardAppCard key={app.id} app={app} index={index} />)}</div> : <p className="rounded-xl border border-dashed border-zinc-700 py-10 text-center text-sm text-zinc-400">Nenhuma aplicação corresponde aos filtros.</p>}
    </>;
}
