...
export function ReleaseUploader({ storeId, productId, disabled }: { storeId: string; productId: string; disabled: boolean }) {
    ...
    return <form onSubmit={submit} className="space-y-3">
        ...
        <textarea value={notes} onChange={(event) => setNotes(event.target.value.slice(0, 20000))} disabled={busy || disabled} maxLength={20000} rows={8} placeholder="Cole aqui as notas ou um template HTML completo do e-mail de atualização (opcional)" className="block w-full rounded-xl border border-zinc-700/80 bg-zinc-900/80 p-3 font-mono text-xs text-zinc-300 placeholder:text-zinc-600" />
