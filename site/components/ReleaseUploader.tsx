...
export function ReleaseUploader({ storeId, productId, disabled }: { storeId: string; productId: string; disabled: boolean }) {
    ...
    return <form onSubmit={submit} className="space-y-3">
        ...
        <textarea value={notes} onChange={(event) => setNotes(event.target.value.slice(0, 5000))} disabled={busy || disabled} maxLength={5000} rows={4} placeholder="Notas da atualização: melhorias, correções e novidades (opcional)" className="block w-full rounded-xl border border-zinc-700/80 bg-zinc-900/80 p-3 text-sm text-zinc-300 placeholder:text-zinc-600" />
