...
export function LojaEditor({ appId, value, onChange, persist, roles = [], channels = [], showProducts = false, productsOnly = false }: { appId: string; value: Aggregate; onChange: (next: Aggregate) => void; persist: (next: Aggregate) => Promise<void>; roles?: DiscordGuildRole[]; channels?: DiscordGuildChannel[]; showProducts?: boolean; productsOnly?: boolean }) {
    ...
    return (
        <div className="space-y-6">
            <nav className="store-section-nav" aria-label="Atalhos da loja">
                <Link href={`/dashboard/${appId}/vendas/produtos`}>Produtos</Link>
                <a href="#personalizar-loja">Personalizar loja</a>
                <a href="#preferencias-loja">Preferências</a>
                <Link href={`/dashboard/${appId}/config/extensions`}>Extensões</Link>
                <Link href={`/dashboard/${appId}/config/mensagens`}>Mensagens</Link>
            </nav>
            ...
            <Section title="Personalização" subtitle="Mensagem de compra e incentivo de avaliação">
                <div className="grid gap-3 md:grid-cols-2">
                    ...
                    <Field label="Fundo da imagem automática" hint="Nova opção do release corrigido V2 · use uma URL direta"><input type="url" className={inputClass} value={str(getPath(doc("personalization"), ["purchase_event", "background"]))} placeholder="https://..." onChange={(e) => setDoc("personalization", setPath(doc("personalization"), ["purchase_event", "background"], e.target.value || null))} /></Field>
