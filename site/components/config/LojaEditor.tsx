...
export function LojaEditor({ value, onChange, persist, roles = [], channels = [], showProducts = false, productsOnly = false }: { value: Aggregate; onChange: (next: Aggregate) => void; persist: (next: Aggregate) => Promise<void>; roles?: DiscordGuildRole[]; channels?: DiscordGuildChannel[]; showProducts?: boolean; productsOnly?: boolean }) {
    ...
    if (productsOnly) return (
        <div className="space-y-6">
            ...
            <Section title="Produtos" subtitle="Preços, estoque, cargos, condições, mensagens e preferências iguais ao painel Discord.">
                ...
                {products.length === 0 ? <Empty text="Nenhum produto cadastrado." /> : <div className="grid gap-3 sm:grid-cols-2">{products.map(({ id, raw }) => { const campos = asRecord(raw.campos); const stock = Object.values(campos).reduce<number>((sum, campo) => sum + stockCount(asRecord(campo).stock), 0); return <button type="button" key={id} onClick={() => setEditingProduct({ id, raw: structuredClone(raw) })} className="flex items-center justify-between gap-3 rounded-xl border border-zinc-800 bg-black/25 p-4 text-left transition hover:border-emerald-500/35"><span className="min-w-0"><b className="block truncate text-sm text-white">{str(raw.name, "Produto sem nome")}</b><small className="mt-1 block text-zinc-500">{Object.keys(campos).length} opção(ões) · {stock} item(ns)</small></span><span className="text-xs font-medium text-emerald-400">Editar tudo</span></button>; })}</div>}
            </Section>
            ...
            <Modal open={Boolean(editingCampo)} onClose={() => setEditingCampo(null)} title={editingCampo ? `Opção: ${str(editingCampo.campo.name, "nova")}` : "Opção"}>{editingCampo ? <CampoModalBody campo={editingCampo.campo} roles={roles} onChange={(campo) => setEditingCampo({ ...editingCampo, campo })} onCommit={commitCampo} onCancel={() => setEditingCampo(null)} /> : null}</Modal>
        </div>
    );
...
function RoleChecklist({ roles, value, onChange }: { roles: DiscordGuildRole[]; value: string[]; onChange: (value: string[]) => void }) {
    ...
    return <div className="max-h-52 space-y-1 overflow-y-auto rounded-xl border border-zinc-800 bg-black/25 p-2">{roles.map((role) => { const checked = selected.has(role.id); return <label key={role.id} className={`flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 text-sm transition ${checked ? "bg-[#5865f2]/15 text-white" : "text-zinc-400 hover:bg-zinc-900"}`}><input type="checkbox" checked={checked} onChange={() => onChange(checked ? value.filter((id) => id !== role.id) : [...value, role.id])} className="h-4 w-4 accent-[#5865f2]" /><span className="min-w-0 flex-1 truncate">{role.name}</span>{role.managed ? <small className="text-[10px] text-zinc-600">integração</small> : null}</label>; })}</div>;
}
