...
export function DeleteApplicationButton({ appId, appName }: { appId: string; appName: string }) {
    ...
    return <><Button size="sm" variant="danger" onClick={() => setOpen(true)}>Excluir</Button><ConfirmDialog open={open} title="Excluir aplicação" message={`A aplicação “${appName}” será removida da infraestrutura e do banco de dados. Esta ação não pode ser desfeita.`} confirmLabel="Excluir definitivamente" danger busy={busy} onConfirm={remove} onCancel={() => !busy && setOpen(false)} /></>;
}
