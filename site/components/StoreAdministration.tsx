...
import { Button, Card, Field, UserChip, inputClass } from "./ui";
...
export function StoreAdministration({ store }: { store: StoreAdministrationView }) {
    ...
    return <div className="space-y-6">
        ...
        <Card><h2 className="font-semibold text-white">Equipe e permissões</h2><p className="mt-1 text-sm text-zinc-500">Autorize outros usuários do Discord a administrar partes da loja.</p>
            ...
            <div className="mt-4 space-y-2">{store.permissions.map((entry) => <div key={entry.userId} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-zinc-800 bg-black/20 p-3"><div><UserChip userId={entry.userId} /><p className="mt-1 text-xs text-zinc-500">{entry.permissions.join(", ") || "Sem permissões"}</p></div><Button size="sm" variant="danger" disabled={pending} onClick={() => run(() => removeStorePermission(store.id, entry.userId), "Acesso removido.")}>Remover</Button></div>)}</div>
        </Card>

        <Card className="border-red-500/20"><h2 className="font-semibold text-red-300">Excluir loja</h2><p className="mt-1 text-sm text-zinc-500">Só é possível excluir uma loja sem aplicações. Digite <strong>{store.name}</strong> para confirmar.</p><div className="mt-4 flex flex-col gap-2 sm:flex-row"><input className={inputClass} value={confirmation} onChange={(event) => setConfirmation(event.target.value)} /><Button variant="danger" disabled={pending || confirmation !== store.name} onClick={() => run(async () => { await deleteStore(store.id, confirmation); router.push("/admin"); }, "Loja excluída.")}>Excluir definitivamente</Button></div></Card>
    </div>;
}
