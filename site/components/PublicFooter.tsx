        ...
        <footer id="suporte" className="public-footer relative overflow-hidden">
            <div className="mx-auto w-full max-w-7xl px-5 py-16 sm:px-8 sm:py-20">
                <section className="footer-cta">
                    <div className="footer-cta-copy"><p className="home-section-index">05 / PRÓXIMO PASSO</p><h2>A operação começa quando você enxerga tudo.</h2><p>Execute a configuração, entre no painel e tire seu projeto do improviso.</p></div>
                ...
                <div className="footer-grid">
                    <div className="footer-brand-block"><Link href="/" aria-label="ZUROS — página inicial" className="inline-flex"><BrandLogo className="h-9 w-36" /></Link><p>Gestão, vendas e aplicações Discord conectadas em uma única experiência.</p><Link href="/status" className="footer-status"><i />Ver status da plataforma</Link></div>
                    ...
                    <nav aria-label="Conta"><h3>Conta</h3><ul><li><Link href={accountHref}>{isAuthenticated ? "Dashboard" : "Entrar"}</Link></li><li><Link href={isAuthenticated ? "/dashboard/store" : "/planos"}>{isAuthenticated ? "Loja" : "Conhecer produtos"}</Link></li><li><Link href={isAuthenticated ? "/dashboard/invoices" : "/login?callbackUrl=%2Fdashboard%2Finvoices"}>{isAuthenticated ? "Faturas" : "Acessar faturas"}</Link></li></ul></nav>
