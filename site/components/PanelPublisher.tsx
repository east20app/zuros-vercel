...
export function PanelPublisher({ appId, panel, channels, initialChannelId, label = "Enviar painel", panelId }: { appId: string; panel: PublishableBotPanel; channels: DiscordGuildChannel[]; initialChannelId?: string; label?: string; panelId?: string }) {
    ...
    return (
        <div className="mt-4 grid gap-2 rounded-xl border border-[var(--accent)]/25 bg-[var(--accent-soft)] p-3 sm:grid-cols-[minmax(0,1fr)_auto]">
            <select aria-label="Canal para publicar o painel" className="h-10 min-w-0 rounded-lg border border-white/10 bg-zinc-950 px-3 text-base text-zinc-200 outline-none focus:border-[var(--accent)]" value={channelId} onChange={(event) => setChannelId(event.target.value)} disabled={!textChannels.length || pending}>
