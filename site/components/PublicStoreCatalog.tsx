...
export function PublicStoreCatalog({ stores, canPurchase = false }: { stores: StoreCatalogDTO[]; canPurchase?: boolean }) {
    ...
    return <div className="plans-catalog-grid" aria-busy={pending}>{entries.map(({ store, product, monthly }) => {
        const features = product.description?.split(/\r?\n|[•;]/).map((item) => item.trim()).filter(Boolean).slice(0, 3) || fallbackFeatures.slice(0, 3);
        const isZurosBot = normalizeName(product.name).includes("zuros bot") || product.productType === "bot";
        return <article key={product.id} className="plans-product-card">
            <div className="plans-product-topline"><span>{store.name}</span></div>
