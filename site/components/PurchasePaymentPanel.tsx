...
function CheckoutSteps({ current }: { current: number }) {
    ...
    return <div className="relative grid grid-cols-3 rounded-2xl border border-white/[.08] bg-[#0b0b0f] px-3 py-5 sm:px-8">
        ...
        {labels.map((label, index) => { const number = index + 1; const done = current > number; const active = current === number; return <div key={label} className="relative z-10 flex flex-col items-center gap-2 text-center"><span className={`grid h-10 w-10 place-items-center rounded-full border text-sm font-bold transition ${done ? "border-emerald-400 bg-emerald-500 text-black" : active ? "border-violet-400 bg-violet-600 text-white shadow-[0_0_22px_rgba(124,58,237,.55)]" : "border-zinc-700 bg-zinc-900 text-zinc-500"}`}>{done ? "✓" : number}</span><span className={active ? "text-xs font-medium text-violet-300" : "text-xs text-zinc-500"}>{label}</span></div>; })}
    </div>;
}

function CheckoutHeading({ eyebrow, title, subtitle }: { eyebrow: string; title: string; subtitle: string }) {
    return <div className="mb-7 text-center"><span className="inline-flex rounded-full border border-violet-500/25 bg-violet-500/10 px-4 py-2 text-sm font-medium text-violet-300">▣ {eyebrow}</span><h1 className="mt-5 text-3xl font-bold tracking-tight text-white sm:text-5xl">{title}</h1><p className="mx-auto mt-3 max-w-2xl text-sm leading-6 text-zinc-400 sm:text-base">{subtitle}</p></div>;
}
