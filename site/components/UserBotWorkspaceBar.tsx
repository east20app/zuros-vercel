...
export function UserBotWorkspaceBar({ routeId, name, productName, active }: { routeId: string; name: string; productName: string; active: boolean }) {
    ...
    return (
        <div className="sticky top-16 z-30 border-b border-white/[.06] bg-[#111214]/90 px-4 backdrop-blur-xl sm:px-8">
            <div className="mx-auto flex max-w-7xl items-center gap-4 overflow-x-auto py-2.5">
                <Link href={`/dashboard/${routeId}`} className="flex shrink-0 items-center gap-2.5 pr-2">
                    <span className="grid h-9 w-9 place-items-center rounded-lg bg-[#5865f2] text-sm font-bold text-white">{name.charAt(0).toUpperCase()}</span>
                ...
                <nav className="flex min-w-max items-center gap-1 border-l border-zinc-800 pl-4" aria-label="Atalhos do bot">
                    {ITEMS.map((item) => {
                        ...
                        return <Link key={item.label} href={href} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition ${selected ? "bg-[#5865f2]/15 text-[#b8bdfb]" : "text-zinc-500 hover:bg-white/[.05] hover:text-white"}`}><Icon name={item.icon} className="h-3.5 w-3.5" />{item.label}</Link>;
                    })}
                </nav>
            </div>
        </div>
