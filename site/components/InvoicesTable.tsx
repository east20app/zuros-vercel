...
export function InvoicesTable({ invoices }: { invoices: UserInvoiceDTO[] }) {
    ...
    return <>
        ...
        {visible.length === 0 ? <Empty icon={<Icon name="invoice" />} title="Você ainda não possui faturas." /> : <div className="invoice-table overflow-x-auto rounded-2xl border border-zinc-800/80"><table className="w-full min-w-[980px] text-left text-sm"><thead className="text-xs uppercase tracking-wide text-zinc-500"><tr><th className="p-4">Aplicação / produto</th><th className="p-4">Plano</th><th className="p-4 text-right">Valor</th><th className="p-4">Tipo</th><th className="p-4">Status</th><th className="p-4 text-right">Ações</th></tr></thead><tbody>{visible.map((invoice) => <tr key={`${invoice.type}-${invoice.id}`} className="border-t border-zinc-900 text-zinc-400 hover:bg-white/[.02]"><td className="p-4"><b className="mb-1 block font-medium text-zinc-200">{invoice.item}</b><TechnicalId value={invoice.id} /></td><td className="p-4 font-medium text-white">{invoice.plan}</td><td className="p-4 text-right font-semibold tabular-nums text-[var(--accent)]">{money.format(invoice.amount)}</td><td className="p-4">{invoice.type === "purchase" ? "Compra" : "Renovação"}</td><td className="p-4"><StatusBadge status={invoice.status} /></td><td className="p-4"><div className="flex flex-wrap justify-end gap-2">{invoice.type === "purchase" ? <a href={`/dashboard/store/cart/${invoice.id}`} className="inline-flex items-center justify-center rounded-lg border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-xs font-semibold text-emerald-300 transition hover:border-emerald-300/60 hover:bg-emerald-400/20">{invoice.paid ? (invoice.delivered ? "Ver pedido" : "Continuar configuração") : "Abrir pagamento"}</a> : null}{invoice.paid ? <Button size="sm" variant="secondary" onClick={() => setReceipt(invoice)}>Ver comprovante</Button> : <span className="self-center text-xs text-zinc-600">Aguardando confirmação</span>}</div></td></tr>)}</tbody></table></div>}
