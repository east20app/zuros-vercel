...
const confirmedAt = new Date();
        ...
        await sendPaymentConfirmedAlert({ userId: String(cart.userId), cartId: input.cartId, type: input.cartType === "renew" ? "renewal" : "purchase", productName: product?.name || application?.name || "Aplicação", applicationName: application?.name, plan: cart.lifetime ? "lifetime" : `${cart.days || 0} dias`, amount: fromCents(expectedCents), expiresAt: application?.expiresAt || (cart.lifetime ? null : undefined) }).catch((error) => console.error("[email] Falha no alerta de pagamento aprovado:", error instanceof Error ? error.message : "erro desconhecido"));
